...
impl CarSharing {
    ...
    pub fn book_car(&mut self, car_id: String, user_id: String, start_time: u64, end_time: u64, deposit: NearToken) -> Result<(), Error> {
        ...
        let rental_fee: NearToken = self.calculate_rental_fee(&car_id.clone(), rental_duration as u32)?;
        let deposit_amount: NearToken = (rental_fee / 10) * 9; // 10% of rental fee
        let attached_deposit: NearToken = attached_deposit();
        // Check if enough deposit was attached
        if attached_deposit < deposit_amount {
        ...
        // Emit event
        app::emit!(Event::CarBooked { car_id, user: user_id.clone(), start_time, end_time, deposit: deposit_amount });
        Ok(())
    }
    pub fn cancel_booking(&mut self, booking_id: String) -> Result<(), Error> {
        if let Some(booking) = self.bookings.remove(&booking_id) {
            let car_id = booking.car_id.clone();
    ...
    pub fn rent_car(&mut self, car_id: String, user_id: String, duration: u32) -> Result<(), Error> {
        ...
        let required_deposit: NearToken = self.calculate_rental_fee(&car_id, duration)?;
        let attached_deposit: NearToken = attached_deposit();
        ...
        self.book_car(car_id.clone(), user_id.clone(), start_time, end_time, attached_deposit)?;
        // Change car status and publish car rental event
        car.available = false;
    ...
    fn calculate_rental_fee(&self, car_id: &str, duration: u32) -> Result<u128, Error> {
        ...
        Ok(total_hours as u128 * car.hourly_rate)
    }
